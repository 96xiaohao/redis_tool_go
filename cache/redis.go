...
func RedisInit(cc *cacheConfig) {
	redisOnce.Do(func() {
		redisInstance = &htRedis{}
		redisInstance.pool = &redis.Pool{
			Dial: func() (redis.Conn, error) {
				c, err := redis.Dial("tcp", cc.Server)
				if err != nil {
					return nil, err
				}
				if _, err := c.Do("AUTH", cc.Password); err != nil {
					c.Close()
					return nil, err
				}
				if _, err := c.Do("SELECT", cc.Database); err != nil {
					c.Close()
					return nil, err
				}
				return c, nil
			...
			IdleTimeout: time.Second * time.Duration(int64(cc.IdleTimeout)),
		}
	})
}
